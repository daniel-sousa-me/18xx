            ...
            if acquired_corp.share_price.liquidation?
              ...
              if debt.positive?
                unless president == @game.share_pool
                  @game.log << "#{president.name} settles #{acquired_corp.name} "\
                               "debts for #{@game.format_currency(debt)}"
          ...
          def settle_shareholders(acquired_corp)
            ...
            if @shareholder_cash.positive?
              ...
              if payouts.any?
                ...
                @log << "#{acquired_corp.name} settles with shareholders "\
                        "#{@game.format_currency(@shareholder_cash)} = "\
