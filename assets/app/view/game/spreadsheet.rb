  ...
  module Game
    class Spreadsheet < Snabberb::Component
      ...
      def render_corporation(corporation, order, index)
        ...
        market_price_text = num_shares_of(@game.share_pool, corporation).to_s
        market_price_text = '*' + market_price_text if corporation.receivership?
        market_price_text += ' + ' + @game.available_shorts(corporation).to_s if @game.respond_to?(:available_shorts)
        children << h('td.padded_number', { style: { borderRight: border_style } }, market_price_text)
