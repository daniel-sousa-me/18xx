  ...
  module Game
    class Spreadsheet < Snabberb::Component
      ...
      def render_corporation(corporation, order, index)
        ...
        extra << h(:td, corporation.loans.size) if @game.total_loans&.nonzero?
        ...
        market_price_text = corporation.share_price ? @game.format_currency(corporation.share_price.price) : ''
        market_price_text += ' + ' + @game.available_shorts(corporation).to_s if @game.respond_to?(:available_shorts)
        children << h('td.padded_number', market_props, market_price_text )
